...
## รูปแบบการโจมตีที่ใช้และประโยชน์ที่ได้

### รูปแบบ: Classic SQL Injection (Authentication Bypass / Data Extraction)
การโจมตีครั้งนี้เป็นแบบ **Classic SQLi** โดยใช้ประโยชน์จากการที่ระบบนำ input ไปต่อใน SQL query โดยตรง

**ประโยชน์ที่ attacker ได้รับ:**
| ประโยชน์ | รายละเอียด |
|---------|-----------|
| ดึงข้อมูล user ทั้งหมด | ได้ชื่อ-นามสกุลของ user ทุกคนในระบบ (admin, Gordon Brown, Hack Me, Pablo Picasso, Bob Smith) |
| รู้ว่าระบบช่องโหว่ | ระบบไม่มีการ validate input ทำให้โจมตีขั้นต่อไปได้ง่าย |
| Bypass เงื่อนไข WHERE | บังคับให้ query return ข้อมูลทุก record โดยไม่ต้องรู้ ID จริง |
| เป็นจุดเริ่มต้นสู่การโจมตีขั้นสูง | สามารถต่อยอดเป็น UNION-based เพื่อดึง password hash ได้ |
